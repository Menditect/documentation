function findCorrectPath(data, version, tag) {
  if (version && tag) {
    ...
    if (!versionLinks) { versionLinks = data.versions['3.2']; } //if the version is not found, default to the current production version
    const path = versionLinks[tag];
    return path;
  } else if (tag) {
    const path = data.nonVersions[tag];
    if (path) return path;
    let versionLinks = data.versions['3.2'];  //if the unversioned tag cannot be found, try to find a versioned tag for the current production version
